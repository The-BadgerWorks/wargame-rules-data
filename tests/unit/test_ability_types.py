# AI-Assisted: Claude Code (model: Claude Sonnet 5) - 010 R13 task 4: failing-first tests for
# `source_class`, the `classify("Psychic", ...)` flip from `DQ-ABILITY-TYPE` to
# `AbilityType.DATASHEET`, the receipt that no published `datasheet:` key moved, and the
# assembly-level `ability_classes` population alongside `ability_keys`.
...
`test_the_old_psychic_behaviour_is_a_dropped_binding` records today's behaviour before the flip:
a `Psychic` row is unmapped in `ABILITY_TYPE_MAP` and raises `DQ-ABILITY-TYPE`, dropping the
binding. Once `pipeline/normalize/ability_types.py` maps `psychic -> AbilityType.DATASHEET`,
that assertion becomes false and is replaced (not duplicated) by
`test_psychic_no_longer_drops_and_resolves_to_datasheet`.
"""
...
def test_psychic_no_longer_drops_and_resolves_to_datasheet() -> None:
    """`psychic -> AbilityType.DATASHEET`; the binding is kept, not dropped."""
    assert classify("Psychic", entity_ref="x") == (AbilityType.DATASHEET, None)
