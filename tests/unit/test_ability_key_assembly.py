...
# AI-Assisted: Claude Code (model: claude-opus-5) - 010 R6 review, findings 1 and 2: the name
# index is a build-level value passed into the assembly, so every call site here builds it from
# the same mapping it assembles from.
...
_CORE_BINDING = f"ds1|1|A1|||{_INVENTED_DESCRIPTION}|Core||\n"
_FACTION_BINDING = f"ds1|1|A2|||{_INVENTED_DESCRIPTION}|Faction||\n"
_UNRESOLVABLE_BINDING = f"ds1|1|A9|||{_INVENTED_DESCRIPTION}|Core||\n"
_NAMED_BINDING = f"ds1|1|A1||Harbour Watch|{_INVENTED_DESCRIPTION}|Datasheet||\n"

_ABILITY_A1 = f"A1|Tidal Step||TF|{_INVENTED_DESCRIPTION}|\n"
_ABILITY_A2 = f"A2|Ember Vigil||TF|{_INVENTED_DESCRIPTION}|\n"
...
def _assemble(detail: dict[str, CsvReadResult]) -> tuple[list[str], list[Finding]]:
    """Assemble one datasheet with the index built from the *same* detail mapping.

    The index is a build-level value the caller passes in (010 R6 review, findings 1 and 2):
    built once, from the whole mapping, never rebuilt per datasheet and never held in module
    state.
    """
...
def test_a_nameless_core_binding_resolves_its_name_through_abilities_csv() -> None:
    keys, findings = _assemble(_detail(bindings=_CORE_BINDING, abilities=_ABILITY_A1))

    assert keys == ["core:tidal-step"]
...
def test_a_nameless_faction_binding_resolves_its_name_through_abilities_csv() -> None:
    keys, findings = _assemble(_detail(bindings=_FACTION_BINDING, abilities=_ABILITY_A2))

    assert keys == ["faction:ember-vigil"]
    assert [finding.finding_code for finding in findings] == []
...
def test_a_bindings_own_name_still_wins_over_the_joined_one() -> None:
    ...
    keys, _findings = _assemble(_detail(bindings=_NAMED_BINDING, abilities=_ABILITY_A1))

    assert keys == ["datasheet:harbour-watch"]


def test_an_unresolvable_nameless_binding_is_reported_not_skipped_in_silence() -> None:
    keys, findings = _assemble(_detail(bindings=_UNRESOLVABLE_BINDING, abilities=_ABILITY_A1))
...
def test_a_detail_source_without_the_abilities_table_reports_rather_than_raising() -> None:
    """A hand-built detail mapping that states only the tables one test needs."""
    detail = _detail(bindings=_CORE_BINDING)
    del detail["Abilities.csv"]

    keys, findings = _assemble(detail)
