...
Kill Team datasheet sees no fabricated "MODELS MAXIMUM" row, and a reviewer reading a regenerated
`data/` tree's diff sees every `006` structure this feature stops silently losing, in the tree's
own bytes, not only in memory.
"""
...
def test_no_header_row_is_produced_while_every_genuine_row_still_resolves() -> None:
    ...
    # Both measured header shapes: the fabricated row is gone, and it is gone via a BLOCKING
    # finding a reviewer cannot silently ignore.
    assert [e.model_name for e in two_row_header] == ["Fenward Sergeant", "Fenward Trooper"]
    assert "CMP-HEADER-ROW" in [f.finding_code for f in two_row_findings]
    assert [e.model_name for e in single_row_header] == ["Fenward Outrider"]
    assert "CMP-HEADER-ROW" in [f.finding_code for f in single_row_findings]

    # The near-miss: every genuine row still resolves, and nothing is refused.
