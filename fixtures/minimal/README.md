...
## One detail-source shape, over the same nine units

`wahapedia/` carries the export. A second `wahapedia-html/` directory carried the same nine
invented datasheets as current-edition datacard pages, so the set could be built twice and the
two curated snapshots compared — the mode-blindness proof (research D1d). 010 R5 deleted the
second acquisition arm, and that directory and its comparison test went with it.
