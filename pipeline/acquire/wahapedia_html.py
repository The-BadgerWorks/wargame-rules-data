...
# AI-Assisted: Claude Code (model: claude-opus-5) - 010 R5: removed per-faction carry-forward
# along with the mechanism itself; an unreachable faction page fails the whole sweep again,
# exactly as FR-008 states it without the declared exemption.
...
def acquire_wahapedia_html(
    ...
    **same** source key. That parity is what makes every stage below ``acquire`` mode-blind — it
    ...
    ``state_path`` (009 rung R05, T090) is accepted and unused: the export-timestamp
    short-circuit is a bulk-export concept — there is no single whole-export change marker a
    per-faction page sweep could probe ahead of itself — but the parameter has to exist here
    too or the two arms would no longer take the same arguments.
    ...
    Which outcome each slug reached is returned via ``coverage`` on the
    :class:`~pipeline.models.source.SourceAcquisition`, never via a changed return shape.
    """
    del state_path
    ...
    try:
        candidate_slugs = enumerate_faction_slugs(active, base)
        ...
        for slug in candidate_slugs:
            try:
                ...
                raise SourceUnreachable(
                    ...
                    "partial sweep is a failed sweep (FR-008)"
                ) from None
            if response.status_code != 200:
                raise SourceUnreachable(
