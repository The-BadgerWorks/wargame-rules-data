...
| `WGC_DETAIL_SOURCE_URL` | *(empty)* | The datasheet-detail source's location: the directory the bulk export's files sit in. There is one detail arm, so there is no shape to select. Must be set for a live acquisition — an unset value is refused by `PipelineConfig.require_detail_source()` with `ExitCode.CONFIG_ERROR` (60) rather than interpreted, because an empty location is a *relative path* and a relative path is the working directory. A fixture build never reads it. The previous-edition and staging trees are refused before a request is constructed, whatever this is set to (`pipeline/acquire/robots.py`). `Last_update.csv` (009 rung R05, FR-030) is one of `pipeline/acquire/wahapedia.py`'s `EXPORT_FILES` — fetched under the same all-or-nothing guarantee as every other table, so its absence is `ExitCode.SOURCE_UNAVAILABLE` (40) exactly as a missing `Datasheets.csv` would be. | Detail-source location; FR-004 permitted path. |
