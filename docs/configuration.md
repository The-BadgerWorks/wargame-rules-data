<!-- AI-Assisted: Claude Code (model: claude-sonnet-5) - Documented every variable in
     ...
     AI-Assisted: Claude Code (model: claude-sonnet-5) - 008 T021: documented
     WGC_RATCHET_TOLERANCE_EQUIPMENT (the two-step ruling's only new variable, Clarifications
...
| `WGC_DETAIL_SOURCE_URL` | *(empty)* | The datasheet-detail source's location, read according to `WGC_DETAIL_ACQUISITION_MODE`: under `csv` it is the directory the export files sit in; under `html` it is the current-edition tree the sweep resolves `SiteMap.xml` and `factions/<slug>/datasheets.html` against (e.g. `https://wahapedia.ru/wh40k11ed`). Must be set for a live acquisition — an unset value is refused with `ExitCode.CONFIG_ERROR` (60) rather than interpreted, because an empty location under `csv` mode is a *relative path* and a relative path is the working directory. A fixture build never reads it. The previous-edition and staging trees are refused before a request is constructed, whatever this is set to (`pipeline/acquire/robots.py`). Under `csv` mode, `Last_update.csv` (009 rung R05, FR-030) is one of `pipeline/acquire/wahapedia.py`'s `EXPORT_FILES` — fetched under the same all-or-nothing guarantee as every other table, so its absence is `ExitCode.SOURCE_UNAVAILABLE` (40) exactly as a missing `Datasheets.csv` would be. | Detail-source location; FR-004 permitted path. |
| `WGC_DETAIL_ACQUISITION_MODE` | `csv` | Which **shape** the detail source is read in: `csv` (the bulk export) or `html` (the current-edition datacard pages). A *variable, never a logic branch* — both arms emit the same `SourceAcquisition` and the same `file name -> CsvReadResult` mapping, so every stage below `acquire` is mode-blind and each grammar, linker and validator is written once. The mode selects a parser; **`WGC_DETAIL_EDITION` is what states which edition the content is**, and the two are set together when the edition is adopted. **The documented default here is `csv`, but every live run overrides it to `html`**: `.env.local` sets it directly, and `candidate.yml`/`publish.yml` both read `vars.WGC_DETAIL_ACQUISITION_MODE` (the repository variable) rather than letting the default apply — `html` is today's wholesale-adoption current-edition path (`004`), not the code's own idea of what should run by default. This is exactly what makes `009-csv-migration`'s FR-011 reversibility a **variable change**, not a code change: flipping the mode back to the documented default (or editing the two overriding locations) reproduces the previous shape's output with no code touched. | Dual-mode detail acquisition (`004` research D1d, FR-003). Reversibility: `009` FR-011, `plan.md` finding 11. |
...
| `WGC_RATCHET_TOLERANCE_OPTIONS` | `0.00` | The proportion `pipeline/validate/gates.py`'s `COV-OPTION-REGRESSION` check allows `loadout.options_resolved` (the proportion of published datasheets whose `wargear_option_state` is `none` or `extracted`) to fall below the previous **published** version's own percent before it raises the blocking finding. Same shape as the four `WGC_RATCHET_TOLERANCE_*` variables above it, joining them unchanged. Deliberately the **only** variable `006-unit-loadout-fidelity` added: the 2026-08-09 clarification ratchets resolved-option coverage with no absolute ceiling, so no separate threshold knob exists for it, and an equipment-vocabulary knob was considered and dropped — the five subject productions cover ≈99.5% of the measured default-equipment sentences with nothing left to tune. **`loadout.default_equipment`'s own ratchet and tolerance variable are `WGC_RATCHET_TOLERANCE_EQUIPMENT`, immediately below** — this row originally said `default_equipment` "carries no ratchet and no tolerance variable in this first extended release, because there is nothing yet to compare it against"; the Product Owner's 2026-08-15 two-step ruling (`008-wargear-option-completion` Clarifications Q2) has since turned that ratchet on, now that two consecutive published versions have reported the baseline `006` D4 was waiting for. | Resolved-option coverage regression tolerance (006 FR-022). |
| `WGC_RATCHET_TOLERANCE_EQUIPMENT` | `0.00` | The proportion `pipeline/validate/gates.py`'s `COV-EQUIPMENT-REGRESSION` check allows `loadout.default_equipment` (the proportion of published datasheets whose `default_equipment_state` is `none` or `extracted`) to fall below the previous **published** version's own percent before it raises the blocking finding — `COV-OPTION-REGRESSION`'s exact symmetric twin, for the second ratcheted loadout figure. Same shape as every `WGC_RATCHET_TOLERANCE_*` variable before it: no absolute ceiling, only a tolerance either side of the previous published percent. The **only** variable `008-wargear-option-completion` adds (Clarifications 2026-08-15 Q2, FR-021) — turning `default_equipment`'s ratchet on takes no new mechanism, only this tolerance knob plus `default_equipment` joining `LOADOUT_RATCHETED_KEYS` and the gate loop iterating it (`pipeline/validate/coverage.py`, `pipeline/validate/gates.py`, `pipeline/cli.py` — this variable is registered ahead of that wiring, in the Foundational phase; the wiring itself is a later phase of the same feature). | Default-equipment coverage regression tolerance (008 FR-021). |
