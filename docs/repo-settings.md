...
| 2026-08-03T13:55Z | Enabled GitHub Pages, `build_type: "workflow"` (GitHub Actions source, per the corrected mechanism in SS3 above) | `html_url: https://the-badgerworks.github.io/wargame-rules-data/`; `status: null` at the time, because no deploy workflow existed yet |
| 2026-08-03 (US4, task T117) | `publish.yml` gained `actions/upload-pages-artifact` + `actions/deploy-pages` steps, run after the checksum and validation gates pass and before the git commit of `site`/`state` -- the previously-deferred step this table's prior entry named | Not yet exercised against the live environment; `docs/verification/approval-rehearsal.md` (task T122) is the first real dispatch and will confirm `status` moves off `null` |
